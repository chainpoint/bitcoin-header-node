...
class HeaderNode extends Node {
   ...
   */
   ...
   startSync() {
    // this normally just fires the pool's sync
    // but that will use the chain's state by default
    // instead we have to write our own that gets the state
    // from the indexer which is saved in the key `R`
    // we'll mimic what resync and sendSync do
    // which is basically just sendLocator with the tip
    // for each peer
    this.pool.startSync();
   }

   // TODO deal with new peers on the pool (setSync etc.)
