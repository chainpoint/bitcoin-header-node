...
const { Chain, Pool, Node } = require('bcoin');
...
class HeaderNode extends Node {
  ...
  init() {
    ...
    this.headerindex.on('error', err => console.log(err));
    this.chain.on('error', err => console.log(err));
  ...
  async startSync() {
    // This should be a custom sync that does a couple things differently
    // 1) we don't need the normal chain locator, just the hash of the first 1-2 blocks
    // 2) we never need to call getBlocks, getHeaders is sufficient
    // Might make sense to make a custom `Pool` that overwrites the resync method
    // and possibly the sendLocator method (where the checkpoints check is made)
