...
const {Chain, Pool, Node, ChainEntry} = require('bcoin');
...
class HeaderNode extends Node {
  ...
  init() {
    ...
    if (this.headerindex)
      this.headerindex.on('error', err => this.error(err));
  ...
  */
  ...
  async setChainTip() {
    ...
    const headerState = await this.headerindex.getState();

    // if everything is fresh, we can sync as normal
    if (!headerState)
      return;

    // if chainTip doesn't match header state height
    // then we can set the chain tip to the header tip
    if (!chainTip || chainTip.height < headerState.height) {
      try {
        const chainState = await this.headerindex.getChainState();
        chainTip = await this.headerindex.getChainTip();
        let entry = ChainEntry.fromRaw(chainTip);
        let hash = entry.tip;
        ...
        // Write initial state.
        // we need to replay all non-historical block entries
        // on the chain so that the chain verification steps
        // will pass when getting new blocks
        const b = this.chain.db.start();
        b.put(key('R').encode(), chainState);
        while (entry.height > this.network.lastCheckpoint) {
          hash = entry.hash;
          this.logger.spam('Adding entry to chain for block: %h', hash);
          // TODO: find a way to make this more future proof
          // since the keys could change
          // also ideally don't want to have to add the hash key pair as well
          // but need this for Chain.getLocator (which is called from Pool's
          // setLoader -> sendSync)
          b.put(key('e', ['hash256']).encode(hash), entry.toRaw());
          b.put(key('H', ['uint32']).encode(entry.height), hash);
          entry = await this.headerindex.getHeaderByHeight(entry.height - 1);
          entry = ChainEntry.fromRaw(entry);
        }

        await this.chain.db.commit();

        await this.chain.reconnect(ChainEntry.fromRaw(chainTip));

        await this.chain.close();
        await this.chain.open();
        await this.chain.db.saveFlags();
        await this.chain.db.saveDeployments();
      } catch (e) {
