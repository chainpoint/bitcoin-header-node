...
/**
 * Retrieve block info from blockcyper
...
async function getRemoteBlockEntries(network, ...heights) {
  ...
  return blocks.map(block => ChainEntry.fromOptions(block).toRaw())
}

/**
 * Because the block data returned from blockcyper
 * does not conform to the same JSON structure as bcoin
 * we need to convert the property names
 * @param {Object} meta - blockcyper returned metadata for one block
 * @returns {Object} block - the bcoin conforming block object
 */
function convertBlockcypherMeta(meta) {
  assert(meta, 'must pass blockcypher response data for conversion')
  const block = {}
