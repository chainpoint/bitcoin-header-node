...
class HeaderHTTP extends HTTP {
  ...
  initRouter() {
    ...
    // get the start block
    this.get('/start', async (req, res) => {
      const start = this.headerindex.startHeight
      if (!start) {
        res.json(404)
        return
      }

      // getEntry will fall back to header if no entry found in chain so this is ok
      const entry = await this.headerindex.getEntry(start)
      entry.height = start
      enforce(entry != null, `Could not find entry for starting height ${start}`)

      // convert to header since this is what is expected by the API
      const header = Headers.fromBlock(entry)
      const json = header.getJSON(this.network, null, start)
      res.json(200, json)
    })
  }
