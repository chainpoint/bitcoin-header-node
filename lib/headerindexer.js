...
const { Indexer, Headers, ChainEntry, Block, CoinView } = require('bcoin')
...
class HeaderIndexer extends Indexer {
   ...
   * Note: This will need to open the database if not already opened
   * _and_ close it again since this operation should typically only be run on initialization
   * and before the parent classe's open method
  ...
  validateStartHeight(height) {
    ...
    assert(
      ...
      `Starting entry height ${height} is too high. Must be no higher than the lastCheckpoint (${lastCheckpoint}) ` +
        `and before a retargetting interval. Recommended max start height: ${this.getHistoricalPoint()}`
    )
    return true
  }
  ...
  /**
   * need to overwrite parent saveGenesis since it only gives a Block meta for indexBlock
   * but we need a full entry
   * and indexBlock needs an entry
   * @private
   * @returns {Promise}
   */
  ...
  //   if (!this.isHistorical(height) || !height) return super._syncChain()
  ...
  //   if (this.startHeight < height) height = this.state.startHeight
   ...
   * will throw if an entry is not in the chain db (when looking up by hash)
   * @param {Number|Buffer} height or hash - block height or hash
   * @returns {Headers|null} block entry
   */
  async getEntry(heightOrHash) {
    let height = heightOrHash
    // if we have a has, we need to retrieve the
    if (Buffer.isBuffer(heightOrHash)) {
      // TODO: It's possible this might not work with our memory-only chain
      const entry = await this.chain.getEntry(heightOrHash)
      assert(entry, `Could not find entry for block %h`, heightOrHash)
      height = entry.height
    }
  ...
  async getTip() {
    ...
    assert(height, 'Cannot get headers tip until indexer has been initailized and synced')
    const tip = await this.getHeader(height)
