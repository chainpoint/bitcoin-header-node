...
class HeaderIndexer extends Indexer {
      ...
      if (this.startHeight && startEntry.height !== this.startHeight)
        throw new Error(
          `Cannot retroactively change start height. Current start height is ${
            startEntry.height
          }. To change the start height delete indexer database otherwise remove start height config to use existing.`
        )
  ...
  /**
   * @private
   * initialize a startTip by running some validations and adding it to the _chain_ db
   * This will validate the startTip argument and add them to the chain db
   * @param {Buffer[]} startTip - an array of at least two raw chain entries
