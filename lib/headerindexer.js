...
class HeaderIndexer extends Indexer {
  ...
  async indexBlock(entry) {
    ...
    else b.put(layout.b.encode(hash), entry.toRaw());

    // could probably avoid saving this by just storing the
    // headers entries by height
  ...
  async getHeaderByHeight(height) {
    ...
    const data = await this.db.get(layout.b.encode(hash));
