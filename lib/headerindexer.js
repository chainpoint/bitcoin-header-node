...
class HeaderIndexer extends Indexer {
  ...
  fromOptions(options) {
    if (!options.network) {
      ...
      // NOTE: This must be the first option set
      // since other configs rely on it
  ...
  async _setStartTip() {
    ...
    // api to retrieve the block header information for mainnet and testnet blocks
  ...
  async _initializeChain() {
    ...
    if (!chainTip || chainTip.height < headersTip.height) {
      ...
      // We will build the chain up from the startHeight to the current headers tip

      // Option 1) Chain initialized with current headers tip if tip is still historical
      // Option 2) If there's no lastCheckpoint (e.g. regtest), start from genesis
      // Option 3) Start from first non-historical block
      let entry
      if (this.isHistorical(headersTip.height)) {
        ...
        this.logger.debug('Headers tip before last checkpoint. Re-initializing chain at tip: %d', headersTip.height)
