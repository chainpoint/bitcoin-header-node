...
class HeaderIndexer extends Indexer {
  ...
  setCustomCheckpoint(height = 0, hash) {
    ...
    this.logger.info(
      'Setting custom lastCheckpoint to %d (checkpoint=%h)',
      height,
      hash
    );
  ...
  async _initializeChain() {
    ...
    if (!chainTip || chainTip.height < headersTip.height) {
      ...
      // initiate the chain to match the headers tip
      let entry;
      ...
      if (
        ...
        this.logger.info(
          'Headers tip before last checkpoint. Re-initializing chain at tip: %d',
          headersTip.height
        );

        // start at one less of the height to set a prevBlock
        entry = await this.getHeaderByHeight(headersTip.height - 1);
      } else {
        // otherwise first entry in the chain should be the last checkpoint
        this.logger.info(
          'Re-initializing chain from last checkpoint: %d',
          this.network.lastCheckpoint
        );
        // TODO: Test if this is necessary if we never run getBlocks from Pool
        // need the chain going back to the last checkpoint because of
        // orphan checks when we run getBlocks when checkpoints gets turned off
        // since the chain history seems to be needed.
        // question is if we can turn off checkpoints without risking orphans and reorgs?
  ...
  async _getLocator(start) {
    let entry;
    if (start == null) {
      entry = await this.getHeaderByHeight(this.state.height);
      start = entry.hash;
    } else {
