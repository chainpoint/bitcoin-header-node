...
class HeaderIndexer extends Indexer {
  ...
  async _setStartTip() {
    ...
    try {
      ...
      let prev = await this.db.get(layout.s.encode(0))
      if (prev) {
        let prevEntry = ChainEntry.fromRaw(prev)
        // if a start height was passed as an option and doesn't match with
        // one saved in DB, throw an error
        if (this.startHeight && prevEntry.height + 1 !== this.startHeight)
          throw new Error(
            `Cannot retroactively change start height. Current start height is ${
              prevEntry.height
            }. Delete indexer database to reset with new start height.`
          )
        else if (this.startHeight) this.logger.debug(`Starting entry already exists for block ${prevEntry.height}.`)
        else this.logger.info(`Starting block for header chain initialized to ${prevEntry.height}`)

        // set start height and tip on the indexer from the db items
        const tip = await this.db.get(layout.s.encode(1))
        // make sure it's a valid ChainEntry
        let tipEntry = ChainEntry.fromRaw(tip)

        // set startTip and startHeight for the indexer
        this.startTip = [prev, tip]
        this.startHeight = tipEntry.height
    ...
    await this.db.put(layout.s.encode(1), entry.toRaw())
    await this.db.put(layout.s.encode(0), prev.toRaw())
  ...
  async _initializeChain() {
    ...
    if (!chainTip || chainTip.height < headersTip.height) {
      ...
      // Option 1) If tip is before historical point, then we need to start the sync over since
      // the chain can't be bootstrapped from historical header data
      ...
      if (this.isHistorical(headersTip.height)) {
        ...
        // resync from startHeight
        for (let data of this.startTip) {
          console.log('entry:', ChainEntry.fromRaw(data))
          await this.addEntryToChain(ChainEntry.fromRaw(data))
        }
        await this.client.chain.close()
        await this.client.chain.open()
        ...
        // TODO: Fix this. Can't just getHeader since this is historical and won't work as an entry
  ...
  async _getLocator(start) {
    ...
    const end = this.getHistoricalPoint() + 1
