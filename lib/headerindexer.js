...
class HeaderIndexer extends Indexer {
  ...
  async _setStartTip() {
    ...
    // we need to make sure to start indexing from no later than the last retarget
    // interval that is before the lastCheckpoint
    this.validateStartHeight(this.startHeight) // will throw on any validation errors
    ...
    // Next, we need to validate and init starting tip in db
    ...
    this.state.height = tip.height
    this.state.startHeight = tip.height
