...
class HeaderIndexer extends Indexer {
  ...
  async _initializeChain() {
    ...
    if (!chainTip || chainTip.height < headerState.height) {
      ...
      this.logger.info(
        'Chain state is behind header. Initializing chain to entry: (height: %d, hash: %h)',
        tip.height,
        tip.hash
      );
      ...
      let entry;
      entry = await this.getHeaderByHeight(tip.height - 1);
      await this.addEntryToChain(entry);
      entry = await this.getHeaderByHeight(tip.height);
      await this.addEntryToChain(entry);
