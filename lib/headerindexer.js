...
class HeaderIndexer extends Indexer {
  ...
  fromOptions(options) {
    ...
    if (options.startHeight) {
      ...
      // i.e. lastCheckpoint - startHeight < retargetInterval
      // we need to make sure to start indexing from the lastRetarget that is
      // before the lastCheckpoint
      const startIsValid = this.verifyStartHeight(options.startHeight)
    ...
    if (options.startTip) {
      ...
      const startEntry = ChainEntry.fromRaw(options.startTip[0])
      const startIsValid = this.verifyStartHeight(startEntry.height)
      ...
      if (!startIsValid)
        throw new Error(
          `Starting entry height ${
            startEntry.height
          } is too high. Must be no higher than the lastCheckpoint (${lastCheckpoint}) and before a retargetting interval. Recommended max start height: ${maxStart}`
        )
  ...
  verifyStartHeight(height) {
    ...
    // regtest and simnet have lastCheckpoint of height zero, in which case we can skip this check
    if (lastCheckpoint)
      assert(
    ...
    // start height can't be later than this amount otherwise pow checks on new blocks will fail
    const maxStart = lastCheckpoint - (lastCheckpoint % retargetInterval)

    if (height > maxStart) return false
  ...
  async _setStartTip() {
    ...
    try {
      ...
      let raw = await this.db.get(layout.s.encode())
      if (raw) {
        let entry = ChainEntry.fromRaw(raw)
        if (this.startHeight && entry.height !== this.startHeight)
          throw new Error(
            `Cannot retroactively change start height. Current start start height is ${
              entry.height
            }. Delete indexer db and try again`
          )
        else if (this.startHeight) this.logger.debug(`Starting entry already exists for block ${entry.height}.`)
        else this.logger.info(`Starting entry for header chain initialized to ${entry.height}`)

        // set start height and tip on the indexer from the db items
        this.startHeight = entry.height
        const next = await this.getHeader(entry.height + 1)
        this.startTip = [entry.toRaw(), next.toRaw()]
    ...
    // can use a custom start height and use the blockcypher
    ...
    // save the start point in the database for starting later
    await this.db.put(layout.s.encode(), entry.toRaw())
  ...
  async _initializeChain() {
    ...
    if (!chainTip || chainTip.height < headersTip.height) {
      ...
      if (this.isHistorical(headersTip.height)) {
        ...
        entry = await this.getHeader(this.network.lastCheckpoint + 1)
      } else {
  ...
  async _getLocator(start) {
    let entry
    if (start == null) {
      entry = await this.client.chain.getEntry(this.state.height)
      // start = entry.hash
    } else {
   ...
   * an ancestor of a checkpoint. This is adapted from the chain's "isHistorical"
   * but to account for custom startHeights. Historical in this case is shifted to be before
   * the last retarget before the lastCheckpoint
  ...
  isHistorical(height) {
    if (this.checkpoints) {
      ...
      const historicalPoint = lastCheckpoint ? lastCheckpoint - (lastCheckpoint % retargetInterval) : 0
